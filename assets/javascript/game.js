// NOTE:: GOING TO REFACTOR CODE BEFORE IMPLEMENTING BLOCKING
...
function unselectFighters(side) {
	fighterNames.filter(x => fighters[x].side === side && fighters[x].inPlay === true).map(unSelect);
}
...
function unhighlightFighters(side) {
	fighterNames.filter(x => fighters[x].side === side && fighters[x].inPlay === true).map(unHighlight);
}
...
function nextTurn() { // add parameter: side, to be used in the various functions for each phase
	...
	targetDiv = document.getElementById("attack-button");
	if (targetDiv !== null) targetDiv.classList.replace("pressed-button","control-button");
	abilityButtonClicked = false;
	targetDiv = document.getElementById("ability-button");
	if (targetDiv !== null) targetDiv.classList.replace("pressed-button","control-button");
...
function clickListener(event) {
	...
	if (clickedValue.attributes[0]) {
		if (clickedValue.attributes[0].value === "next-turn" && !targetedAbility) {
				clickedValue.classList.replace("control-button2","pressed-button");
				currentSideTurn = "sith";
				textElements.turnTrackerText.textContent = currentSideTurn;
				attackCadre = []; textElements.notificationText.textContent = "Opponent's Turn";
				nextTurn();
		}
			...
			if (attackButtonClicked === false) {
				...
				targetDiv = document.getElementById("send-button");
				if (targetDiv !== null && targetDiv.classList.contains("invisible")) targetDiv.classList.remove("invisible");
				...
				attackButtonClicked = false;
				clickedValue.classList.replace("pressed-button","control-button"); 
				...
				for (x = 0; x < fighterNames.length; x++) {
					if (roundCounter >= fighters[fighterNames[x]].rank && fighters[fighterNames[x]].inPlay === true && fighters[fighterNames[x]].tapped === false) {
						...
						targetDiv = document.getElementById("send-button");
						if (targetDiv !== null && !targetDiv.classList.contains("invisible")) targetDiv.classList.add("invisible"); 
					}
				}
			}
		}
